  ...
  return (
    <section className="flex flex-col gap-10  bg-slate-50">
      <div className="flex items-center gap-3">
        <Link href={`/${locale}/products`} className="rounded-2xl bg-rose-500 px-4 py-2 text-white">
          {t('cta.explore')}
        </Link>
      ...
      <div>
        <h2 className="mb-4 text-2xl text-[#461128] font-semibold">{t('home.featured')}</h2>
        {featured.length === 0 ? (
          ...
          <Link href={`/${locale}/products`} className="text-sm text-slate-600 hover:underline">
          {t('home.viewAll')}
        </Link>
      </div>
    </section>
